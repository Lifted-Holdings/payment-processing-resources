...
This repository accepts only statement-level aggregates. It is not designed to ingest source statements, cardholder data, bank data, merchant-owner data, credentials, or secrets. Automated pattern screening is defense in depth; it cannot prove arbitrary prose contains no identifying or confidential information. Human review remains mandatory before any record is made public. The validator's success output says exactly this, and never reports that a record satisfies privacy rules.

Screening normalizes text with NFKC and treats any Unicode space, punctuation, or format character as a digit separator before confirming a card-number candidate with Luhn, so common transcription and paste artifacts do not defeat it. Two deliberate limits follow from keeping false positives out of ordinary fee prose: an ending phrase followed by a four-digit calendar year is read as a reporting period unless the text also names a card or account number, and digit groups shorter than three are not joined across punctuation. Screening cost is bounded linearly in the length of the text so that a large in-limit record cannot stall validation.

The validator verifies the schema file against the SHA-256 declared for it in `checksums.txt` before using it, and refuses to certify any record when the two disagree. A packaged or offline copy that ships no `checksums.txt` reports its schema as `undeclared` in the corpus report and continues; absence of the declaration is not treated as proof of integrity.
